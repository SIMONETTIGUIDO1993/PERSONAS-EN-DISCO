...
let cantidadDePersonas: number[] = new Array(270);
...
// carga de datos
for (let i: number = 0; i < cantidadDePersonas.length; i++) {
  cantidadDePersonas[i] = aleatorio(edadMinima, edadMaxima);
}
// recorro y calculo rangos

for (let i: number = 0; i < cantidadDePersonas.length; i++) {
  if (cantidadDePersonas[i] < 21) {
    menorDe21 += 1;
  } else {
    mayorDe21 += 1;
  }
...
console.log(
  `Menores de 21 años: ${menorDe21}, Mayores o iguales a 21:${mayorDe21} y en total son: ${cantidadDePersonas.length}`
);
